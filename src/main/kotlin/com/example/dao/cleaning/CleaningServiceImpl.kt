...
class CleaningServiceImpl : CleaningService {
    ...
    override suspend fun addCleaning(cleanerId: Int, roomId: Int, hotelId: Int): Cleaning? = DatabaseFactory.dbQuery {
        logger.debug { "add cleaning for user $cleanerId, hotel $hotelId and room $roomId" }

        val cleanerInfo = CleanerInfo.find { CleanerInfos.id eq cleanerId }.singleOrNull() ?: error("User $cleanerId doesn't have cleanerInfo or doesn't exists")
